...
class PostEffectBase(MonoBehaviour):
	...
	def CheckShaderAndCreateMaterial(s as Shader, m2Create as Material) as Material:
		if not s:
	...
	def CreateMaterial(s as Shader, m2Create as Material) as Material:
		if not s:
			Debug.Log("Missing shader in $(ToString())")
			return null

		if m2Create and m2Create.shader == s and s.isSupported:
			return m2Create
	...
	def CheckSupport() as bool:
		return CheckSupport(false)

	virtual def CheckResources() as bool:
	...
	def CheckSupport(needDepth as bool) as bool:
		isSupported = true
	...
	def CheckSupport(needDepth as bool, needHdr as bool) as bool:
	...
	def CheckShader(s as Shader) as bool:
		Debug.Log("The shader $(s.ToString) on effect $(ToString()) is not of the Unity 3.2+ effects suite anymore.")
		if not s.isSupported:
			NotSupported()
		return false
